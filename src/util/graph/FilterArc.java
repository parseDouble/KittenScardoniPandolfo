 ...
 * belong to a given <i>filtering</i> set.
 *
 * @author <A HREF="mailto:fausto.spoto@univr.it">Fausto Spoto</A>
 */

final class FilterArc<X> extends Arc<X> {
     ...
     * @param filter the set of elements which can be propagated from the
     *        <tt>source</tt> to the <tt>sink</tt>
     */

    FilterArc(Node<X> source, Node<X> sink, BitSet<X> filter) {
	super(source,sink);
	...
	if (!(other instanceof FilterArc)) return false;

	return super.equals(other) && filter == ((FilterArc)other).filter;
    }
	...
	getSource().propagate(getSink(),workSet,sets,filter);
    }
	...
	return super.toString() + " only in " + filter;
    }
}
