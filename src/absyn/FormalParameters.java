...
public class FormalParameters extends Absyn {
     ...
     * It type-checks the first parameter and then, recursively, the
     * <tt>next</tt> parameters, if any.
     *
	...
	if (next != null) result = next.typeCheck();
     ...
     * the parameters in the symbol table of the type-checker, with their
     * declared type.
     *
     * @param checker the type-checker to be used for type-checking
     * @return a type-checker derived from <tt>checker</tt>, but
	...
	if (next != null) checker = next.typeCheck(checker);
     ...
     * recursively, those of the <tt>next</tt> parameters. The
     * difference with <tt>typeCheck()</tt> is that class types occurring in
     * these parameters are not type-checked.
     *
     * @return the list of semantical types of these parameteres.
     */
	...
	if (next != null) result = next.toType();
	else result = TypeList.EMPTY;
